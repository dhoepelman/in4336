...
The disadvantages of this problem are clearly visible.
$\mli{OPT}-\mli{LB}$ iterations are need which is especially problematic if $\mli{OPT}$ is large

An alternative approach is too determine increasingly better lower and upper bounds for the cost.
...
\subsection{SMT}
As SMT is a generalization to SAT, it can also solve decision problems. 
The problem formulation of GC into SMT using propositional logic has also been adopted from \cite{cs395tGCtoSAT}.
...
\section{RCPSP}
The Resource Constraint Project Scheduling Problem (RCPSP) is the problem of finding a schedule for a 
set of n jobs \emph{V} = {1,...,n} that have precedence and resource contraints with the goal to 
...
\subsection{ILP}
The 0-1 ILP model for RCPSP is formulated as in \cite{horbach2010boolean} and in 
$\cite{mingozzi1998exact}. For each activity (job) \emph{i} \in \emph{V} and for each period \emph{t} \in \emph{$es_i,...,ls_i$} a binary variable \emph{$x_{it}$} indicates wether an activity \emph{i} starts at period \emph{t} (\emph{$x_{it}$} = 1) or not (\emph{$x_{it}$} = 0). \emph{Same as with SAT, we determine for each activity \emph{i} its earliest (\emph{$es_i$}) and its latest (\emph{$ls_i$}) start $
$times, its earliest (\emph{$ef_i$}) and its latest (\emph{$lf_i$}) finish times, using the critical-path $
$method by Floyd-Warshall algorithm}. With the variables \emph{$u_{it}$} we can indicate for each activity $
$\emph{i} if it is executing at period \emph{t} (\emph{$u_{it}$} = 1) or not executing (\emph{$u_{it}$} = 0).$
$The resulting 0-1 ILP project makespan optimization in formal description listed below.$
\begin{subequations}
\begin{align*}
        & & & min \emph{x_{n+1},T} & & \emph{i} \in \emph{V} \\
        &  (1) & & \sum\limits_{\emph{t}\in\emph{es_i,...,ls_i}} \emph{x_{it}} = 1 & & \emph{i} \in \emph{V}, \emph{t} \in \emph{es_i,...,ls_i}, \emph{t} \in \emph{es_i,...,ls_i} + \emph{d_i} - 1 \\
        &  (2) & & \sum\limits_{\emph{l}\in\emph{es_j,...,t-d_j}} \emph{x_{jl}} \geq \emph{x_{it}} & & \emph{(j,i)} \in \emph{E}, \emph{t} \in \emph{es_i,...,ls_i} \\
        &  (3) & & \sum\limits_{\emph{i}\in\emph{V,}\emph{es_i}\leq\emph{t}\leq\emph{ls_i}} \emph{r_{ik}}\emph{u_{it}} \leq \emph{R_k} & & \emph{t} \in {0,1,...,\emph{T}}, \emph{k} \in \emph{R} \\
        &  (4) & & \emph{x_{ik}} + \emph{x_{jk}} \leq 1 & & (i,j) \in E, \emph{k} = 1,...,\emph{n} \\
        &  (5) & & \emph{x_{00}} = 1 & & \\
        &  (6) & & \emph{x_{it}} \in {0, 1} & & \\
        &  (7) & & \emph{u_{it}}, \emph{x_{it}} \in \mathbb{Z} & &
\end{align*}
\end{subequations}
